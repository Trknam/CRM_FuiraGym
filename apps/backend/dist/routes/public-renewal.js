...
exports.publicRenewalRoutes = void 0;
const express_1 = require("express");
const prisma_1 = require("../db/prisma");
const email_1 = require("../services/email");
const renewal_token_1 = require("../services/renewal-token");
exports.publicRenewalRoutes = (0, express_1.Router)();
...
// });
function qrUrl(bankId, accountNo, accountName, amount, addInfo) {
...
async function getPaymentQr(branchId, amount, memberCode) {
...
async function renewAfterPayment(paymentId, membershipId) {
    const now = new Date();
    return prisma_1.prisma.$transaction(async (tx) => {
        ...
        const newEndDate = new Date(baseDate.getTime() + membership.package.durationDays * 24 * 60 * 60 * 1000);
...
exports.publicRenewalRoutes.post("/:membershipId", async (req, res) => {
    try {
        const membershipId = String(req.params.membershipId ?? "").trim();
        const token = String(req.body?.token ?? "").trim();
        const action = String(req.body?.action ?? "").trim().toLowerCase();
        const verified = (0, renewal_token_1.verifyRenewalToken)(token, membershipId);
        if (!verified)
            return res.status(400).json({ message: "Liên kết gia hạn không hợp lệ hoặc đã hết hạn." });
        ...
        const qr = await getPaymentQr(membership.member.branchId, Number(membership.package.price), membership.member.memberCode);
        return res.json({
            data: {
...
exports.publicRenewalRoutes.post("/:membershipId/payment", async (req, res) => {
    try {
        const membershipId = String(req.params.membershipId ?? "").trim();
        ...
        const verified = (0, renewal_token_1.verifyRenewalToken)(token, membershipId);
        if (!verified)
            return res.status(400).json({ message: "Liên kết gia hạn không hợp lệ." });
        const payment = await prisma_1.prisma.payment.findFirst({
            where: { id: paymentId, membershipId, note: { startsWith: "RENEWAL:" } },
        });
        if (!payment)
            return res.status(404).json({ message: "Không tìm thấy giao dịch gia hạn." });
        if (payment.expiresAt && payment.expiresAt.getTime() <= Date.now() && payment.status === "PENDING") {
            await prisma_1.prisma.payment.update({ where: { id: payment.id }, data: { status: "CANCELLED" } });
        ...
        if (!result.alreadyRenewed) {
            await (0, email_1.sendMemberBrandedEmail)(result.membership.member.email, result.membership.member.fullName, "Gia hạn gói tập thành công", "<p>Gói <b>" + result.membership.package.name + "</b> của anh/chị đã được gia hạn thành công.</p>" +
