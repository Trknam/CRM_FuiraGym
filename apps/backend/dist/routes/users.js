...
exports.usersRoutes.post("/", async (req, res) => {
    try {
        ...
        if (!fullName || !identifier || password.length < 8)
            return res.status(400).json({ message: "Họ tên, tài khoản và mật khẩu tối thiểu 8 ký tự là bắt buộc." });
...
exports.usersRoutes.patch("/", async (req, res) => {
    try {
        ...
        if (id === current.id && data.role && data.role !== "SUPER_ADMIN")
            return res.status(400).json({ message: "Không thể hạ quyền tài khoản quản trị đang đăng nhập." });
        if (existing.role === "SUPER_ADMIN" && data.isActive === false) {
            ...
            if (activeAdmins <= 1)
                return res.status(400).json({ message: "Phải giữ lại ít nhất một tài khoản quản trị hoạt động." });
        }
