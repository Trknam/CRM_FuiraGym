...
export function RegisterForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setLoading(true);
    try {
      const response = await fetch("/api/auth/register", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(input) });
      const data = await response.json();
      if (!response.ok) { setFormError(data.message ?? "Đăng ký thất bại."); setErrors(data.errors ?? {}); return; }
      window.location.href = "/";
    } catch { setFormError("Không thể kết nối đến máy chủ."); }
...
function PasswordField({ label, name, error, visible, onToggle }: { label: string; name: string; error?: string; visible: boolean; onToggle: () => void }) {
  return <label className="block text-sm font-semibold"><span>{label}</span><span className="relative mt-2 block"><input name={name} type={visible ? "text" : "password"} className={`field pr-11 ${error ? "border-[var(--danger)]" : ""}`} autoComplete="new-password" /><button type="button" aria-label={visible ? "Ẩn mật khẩu" : "Hiện mật khẩu"} onClick={onToggle} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg p-2 text-[var(--muted)] hover:bg-[var(--surface-subtle)]">{visible ? <EyeOff size={17} /> : <Eye size={17} />}</button></span>{error && <span className="mt-1 block text-xs font-normal text-[var(--danger)]">{error}</span>}</label>;
}
