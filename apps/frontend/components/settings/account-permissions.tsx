...
export function AccountPermissions() {
    ...
    async function load() {
        ...
        try {
            const [u, p] = await Promise.all([
                ...
                fetch("/api/users/permissions", { credentials: "include", cache: "no-store" }),
            ]);
    ...
    const staffPermissions = useMemo(
        () => roles.find((r) => r.role === "STAFF")?.permissions ?? [],
        [roles],
    );
    ...
    function openCreate() {
        setEditing(null);
        setCreating(true);
        setForm({ fullName: "", identifier: "", password: "", role: "STAFF", isActive: true });
    }
    ...
    async function save() {
        ...
        if (
            creating &&
            (!form.fullName.trim() || !form.identifier.trim() || form.password.length < 8)
        ) {
            setError("Vui lòng nhập đầy đủ họ tên, tài khoản và mật khẩu tối thiểu 8 ký tự.");
            return;
        }
        try {
            ...
            setError(e instanceof Error ? e.message : "Không thể lưu tài khoản.");
        }
    }
    ...
    async function lockUser(user: UserRow) {
        ...
        try {
            ...
            setError(e instanceof Error ? e.message : "Không thể khóa tài khoản.");
        }
    }
        ...
        <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p className="mt-1 text-sm text-[var(--muted)]">
                        Chỉ quản trị viên có quyền quản lý tài khoản nhân viên.
                    </p>
                </div>
                                    ...
                                    >
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-3">
                                                <span className="rounded-lg bg-[var(--surface-subtle)] p-2">
                                                    <UserRound size={16} />
                                                </span>
                                                <div>
            ...
            <div className="card p-5">
                <div className="mb-4 flex items-center gap-2">
                    <ShieldCheck size={19} />
                    <h2 className="font-bold">Quyền mặc định của nhân viên</h2>
                </div>
                <div className="grid gap-4 md:grid-cols-2">
                    {Object.entries(groupedPermissions).map(([group, permissions]) => (
                        <div key={group} className="rounded-xl border border-[var(--border)] p-4">
