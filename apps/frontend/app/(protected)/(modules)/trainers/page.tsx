...
export default function TrainersPage() {
    ...
    async function save(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            const response = await fetch("/api/trainers", {
                ...
                body: JSON.stringify(
                    editing ? { id: editing.id, ...form, status: editing.status } : form,
                ),
            });
            const result = await response.json();
    ...
    async function remove(id: string) {
        if (
            !window.confirm(
                "Trainer này sẽ được chuyển sang trạng thái Tạm nghỉ. Bạn có chắc không?",
            )
        )
            return;
        setError("");
    ...
    const filtered = trainers.filter((trainer) =>
        [trainer.name, trainer.email, trainer.phone, trainer.specialty].some((value) =>
                ...
                <section className="card overflow-hidden">
                    <div className="flex flex-col gap-3 border-b border-[var(--border)] p-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="text-xs text-[var(--muted)]">
                            {filtered.length} / {trainers.length} Trainer trong DB
                        </div>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                            ...
                            <table className="w-full min-w-[900px] text-left text-sm">
                                ...
                                <tbody>
                                    {filtered.map((trainer) => (
                                        <tr className="table-row" key={trainer.id}>
                                            ...
                                            <td>
                                                <span
                                                    className={`badge ${trainer.status === "Đang hoạt động" ? "badge-success" : "badge-neutral"}`}
                                                >
                                                    {trainer.status}
                                                </span>
                                            </td>
                        ...
                        {!loading && filtered.length === 0 && (
                            <div className="p-12 text-center text-sm text-[var(--muted)]">
                                Chưa có Trainer trong PostgreSQL.
                            </div>
                        )}
                    </div>
                </section>
                    ...
                    >
                        <div className="mb-6 flex items-center justify-between">
                            <div>
                                <h2 className="text-lg font-bold">
                                    {editing ? "Sửa Trainer" : "Thêm Trainer"}
                                </h2>
                                <p className="mt-1 text-xs text-[var(--muted)]">
                                    {editing
                                        ? "Thay đổi sẽ được cập nhật vào PostgreSQL."
                                        : "PT mới sẽ được lưu trực tiếp vào PostgreSQL."}
                                </p>
                            </div>
                            <button
                                type="button"
                        ...
                        <div className="mt-6 flex justify-end gap-2">
                            ...
                            <button disabled={saving} className="btn btn-primary" type="submit">
                                {saving ? "Đang lưu..." : editing ? "Lưu thay đổi" : "Tạo Trainer"}
                            </button>
                        </div>
                    </form>
                </div>
